...
int g_filesize = 0;
BitSequence *g_fileCharStream = NULL;
BitSequence *g_fileBitStream = NULL;
...
int    readRandBinary(char* filename)
{
	...
	nFileLen = ftell(fp);
	g_filesize = nFileLen;
	g_fileCharStream = (BitSequence*)malloc(nFileLen*sizeof(char));
	g_fileBitStream = (BitSequence*)malloc(nFileLen*8*sizeof(char));

	fseek(fp,0,SEEK_SET);
	fread(g_fileCharStream,nFileLen,1,fp);

	converCharToBits(g_fileCharStream,nFileLen, g_fileBitStream);
...
T_LONGLONG getTickCount()
{
	...
	gettimeofday(&timenow, &tmpzone);

	T_LONGLONG uRet;
	uRet = ((T_LONGLONG)(timenow.tv_sec)*1000*1000 
			+ (T_LONGLONG)(timenow.tv_usec));
...
openOutputStreams(int option)
{
	...
	char    cmdfreqfn[200];
	char    cmdstatsDir[200];

	
	sprintf(freqfn, "experiments/%s/freq.txt", generatorDir[option]);
	sprintf(cmdfreqfn,"mkdir -p experiments/%s",generatorDir[option]);
	system(cmdfreqfn);
	...
	for( i=1; i<=NUMOFTESTS; i++ ) {
		if ( testVector[i] == 1 ) {
			...
			sprintf(cmdstatsDir, "mkdir -p experiments/%s/%s", generatorDir[option], testNames[i]);
			system(cmdstatsDir);
	...
	//modify by lfm 按照gmb的样本给1000
	tp.numOfBitStreams = g_filesize*8;
	printf("\n");
}
